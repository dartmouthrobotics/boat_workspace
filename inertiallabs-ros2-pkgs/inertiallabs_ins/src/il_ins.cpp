...
struct Context {
    ...
    rclcpp::Time time_initial;
    double time_diff = 0.02;
...
void publishDevice(IL::INSDataStruct* data, void* contextPtr)
{
    ...
    if (!context->time_initialized){
        ...
        context->time_initial = timestamp;
        context->time_diff += data->GPS_IMU_Time;
        context->time_initialized = true;
    }
    else{
        timestamp = context->time_initial + rclcpp::Duration::from_seconds(data->GPS_IMU_Time - context->time_diff);
    }
    ...
    if (context->sensorDataPub->get_subscription_count() > 0)
    {
        ...
        // FIX: was all [0], now correctly [0],[1],[2]
